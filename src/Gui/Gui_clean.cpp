...
// Aqui você deve incluir seus headers personalizados, fonts, opções etc
...
namespace Gui
{
    ...
    void Render()
    {
        ...
        if (!Global::LoggedIn)
        {
            ...
            static bool initializedLicense = false;
            if (!initializedLicense) {
                std::ifstream infile("license.txt");
                if (infile.good()) {
                    std::string savedKey;
                    std::getline(infile, savedKey);
                    strncpy_s(licenseKey, savedKey.c_str(), sizeof(licenseKey) - 1);
                }
                initializedLicense = true;
            }

            ImGui::SetNextWindowSize(ImVec2(400, 320));
            ImGui::SetNextWindowPos(ImVec2(ImGui::GetIO().DisplaySize.x * 0.5f - 200, ImGui::GetIO().DisplaySize.y * 0.5f - 160));
            if (ImGui::Begin(xorstr("##LoginWindow"), nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse))
            {
                ...
                // Premium Header
                draw_list->AddRectFilled(p, ImVec2(p.x + s.x, p.y + 40), ImColor(30, 30, 35), 8.0f, ImDrawFlags_RoundCornersTop);
                ImGui::SetCursorPos(ImVec2(10, 10));
                ImGui::PushFont(InterBold);
                ImGui::TextColored(ImColor(184, 51, 142), xorstr("FLUFFY-FIVEM LOADER"));
                ImGui::PopFont();

                ImGui::SetCursorPos(ImVec2(10, 50));
                ImGui::PushFont(InterRegular);
                ImGui::TextColored(ImColor(200, 200, 200), xorstr("Welcome back!"));
                
                ImGui::SetCursorPos(ImVec2(10, 75));
                ImGui::TextColored(ImColor(100, 100, 100), (std::string(xorstr("Version: ")) + Core::Update::CurrentVersion).c_str());
                if (Core::Update::UpdateAvailable) {
                    ImGui::SameLine();
                    ImGui::TextColored(ImColor(255, 50, 50), (std::string(xorstr(" -> Update Available: ")) + Core::Update::LatestVersion).c_str());
                }

                ImGui::SetCursorPos(ImVec2(10, 120));
                ImGui::Text(xorstr("LICENSE KEY"));
                ImGui::SetNextItemWidth(-10);
                ImGui::InputText(xorstr("##KeyInput"), licenseKey, sizeof(licenseKey), ImGuiInputTextFlags_Password);

                ImGui::SetCursorPos(ImVec2(10, 180));
                if (ImGui::Button(xorstr("LOGIN"), ImVec2(s.x - 20, 35)))
                {
                ...
                if (Core::Update::UpdateAvailable) {
                    ...
                    if (ImGui::Button(xorstr("DOWNLOAD UPDATE"), ImVec2(s.x - 20, 35))) {
                        ShellExecuteA(NULL, "open", Core::Update::UpdateUrl.c_str(), NULL, NULL, SW_SHOWNORMAL);
                    }
                }

                if (!KeyAuthApp.response.success && strlen(licenseKey) > 0) {
                    ImGui::SetCursorPos(ImVec2(10, s.y - 30));
                    ImGui::TextColored(ImColor(255, 50, 50), KeyAuthApp.response.message.c_str());
                }
        ...
        if (ImGui::Begin("##MainWindow", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse))
        {
            ...
            ImGui::PushFont(InterBold);
            draw_list->AddText(ImVec2(winPos.x + 15, winPos.y + (headerHeight - ImGui::CalcTextSize(xorstr("FLUFFY-EXTERNALS")).y) / 2.0f),
            ImGui::GetColorU32(ImVec4(184 / 255.f, 51 / 255.f, 142 / 255.f, 1.0f)), xorstr("FLUFFY-EXTERNALS"));
            ImGui::PopFont();
            ImGui::PopFont();
            ...
            ImGui::SetCursorPos(ImVec2(10, winSize.y - footerHeight));
            ImGui::Separator();
            ImGui::PushFont(InterRegular);
            ImGui::TextColored(ImColor(150, 150, 150), xorstr("User: "));
            ImGui::SameLine();
            ImGui::TextColored(ImColor(255, 255, 255), KeyAuthApp.user_data.username.c_str());

            std::string expiryStr = "Lifetime";
            if (!KeyAuthApp.user_data.subscriptions.empty()) {
                long long exp = std::stoll(KeyAuthApp.user_data.subscriptions[0].expiry);
                if (exp > 0 && exp < 2000000000) { // arbitrary limit for non-lifetime
                    time_t now = time(0);
                    long long diff = exp - now;
                    if (diff > 86400) expiryStr = std::to_string(diff / 86400) + " Days Ref";
                    else if (diff > 3600) expiryStr = std::to_string(diff / 3600) + " Hours Ref";
                    else if (diff > 0) expiryStr = "Soon";
                    else expiryStr = "Expired";
                }
            }
            ImGui::TextColored(ImColor(150, 150, 150), xorstr("Expiry: "));
            ImGui::SameLine();
            ImGui::TextColored(ImColor(184, 51, 142), expiryStr.c_str());
            ImGui::PopFont();
                    ...
                    ImGui::BeginGroup();
                    {
                        ImGui::CustomChild(xorstr("Settings"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 330));
                        {
                            ...
                            if (g_Config.ESP->DistanceFromMe) {
                                ...
                                ImGui::AlignTextToFramePadding();
                                ImGui::SameLine();
                                ImGui::SetCursorPosX(fixedX); // Espaço de 5px à direita do texto
                                ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 1); // Espaço de 5px à direita do texto
            ...
            case MISC_EXPLOITS:
                ...
                ImGui::BeginGroup();
                {
                    ...
                    ImGui::BeginGroup();
                    {
                        ImGui::CustomChild(xorstr("Settings"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 220));
                        {
                            ...
                            ImGui::PushFont(InterRegular);
                            if (ImGui::Button(xorstr("Fix Vehicle"), ImVec2(-1, 28)))
                                Core::SDK::Pointers::pLocalPlayer->GetLastVehicle()->Fix();
                        }
                        ImGui::EndCustomChild();

                        // --- Invisibility ---
                        ImGui::CustomChild(xorstr("Invisibility"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 100));
                        {
                            if (ImGui::Checkbox(xorstr("Invisible (Alpha 0)"), &Core::Features::cInvisibility::Enabled))
                            {
                                Core::Features::cInvisibility::Apply(Core::Features::cInvisibility::Enabled);
                            }
                            ImGui::TextColored(ImColor(150, 150, 150), xorstr("Hides your player model."));
                            ImGui::TextColored(ImColor(150, 150, 150), xorstr("Server may reset visibility."));
                        }
                        ImGui::EndCustomChild();
                    }
                ...
                ImGui::BeginGroup();
                {
                    ImGui::CustomChild(xorstr("Players List"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 330));
                    {
                        ...
                        for (int i = 0; i < Core::SDK::Game::EntityList.size(); i++)
                        {
                            ...
                            if (ImGui::Selectable(Core::SDK::Game::EntityList[i].NetworkInfo.UserName.c_str(), &IsSelected)) SeletedIndex = i;
                        }
                        ImGui::PopStyleVar();
                    ...
                    ImGui::BeginGroup();
                    {
                        ...
                        ImGui::CustomChild(xorstr("Actions"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 180));
                        {
                            ...
                            else
                            {
                                ...
                                // --- Spectate ---
                                bool spectating = Core::Features::cSpectate::Enabled &&
                                                  Core::Features::cSpectate::TargetPed == SelectedPed.Ped;
                                if (ImGui::Checkbox(xorstr("Spectate"), &spectating))
                                {
                                    if (spectating) {
                                        Core::Features::cSpectate::TargetPed = SelectedPed.Ped;
                                        Core::Features::cSpectate::Enabled = true;
                                    } else {
                                        Core::Features::cSpectate::Stop();
                                    }
                                }

                                ImGui::Spacing();
                                ImGui::PushFont(InterRegular);
                                if (ImGui::Button(xorstr("Teleport To"), ImVec2(-1, 28)))
                                {
                    ...
                    ImGui::CustomChild(xorstr("Vehicles List"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 330));
                    {
                        ...
                        ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(14, 8));
                        {
                            ...
                            for (int i = 0; i < Core::SDK::Game::VehicleList.size(); i++)
                            {
                                ...
                                if (ImGui::Selectable(Name.c_str(), &IsSelected)) SeletedIndex = i;
                            }
                        }
            ...
            case MISC_TELEPORT:
                ...
                ImGui::BeginGroup();
                {
                    ImGui::CustomChild(xorstr("Teleport List"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 330));
                    {
                        ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(14, 8));
                    ...
                    ImGui::BeginGroup();
                    {
                        ...
                        ImGui::CustomChild(xorstr("Actions"), ImVec2(ImGui::GetWindowSize().x / 2 - 25, 330));
                        {
                            ...
                            ImGui::PushFont(InterRegular);
                            if (ImGui::Button(teleportButtonText.c_str(), ImVec2(-1, 30))) {
                                if (selectedItem == 0) {
            ...
            case MISC_TRIGGERFINDER:
                ...
                ImGui::BeginGroup();
                {
                    ...
                    {
                        ImGui::TextColored(ImColor(180, 220, 255), xorstr("Finds Lua TriggerServerEvent strings"));
                        ImGui::TextColored(ImColor(150,150,150), xorstr("in FiveM memory (payment, weapons, jobs etc)"));
                        ImGui::Spacing();
            ...
            case MISC_LUAEXECUTOR:
                ...
                ImGui::BeginGroup();
                {
                    ImGui::CustomChild(xorstr("Lua Executor"), ImVec2(ImGui::GetWindowSize().x - 40, 330));
                    {
                        ImGui::TextColored(ImColor(100, 200, 255), xorstr("External Script Analysis & Trigger Simulation"));
                        ImGui::Separator();

                        static char buf[1024] = "";
                        ImGui::InputTextMultiline(xorstr("##Source"), buf, IM_ARRAYSIZE(buf), ImVec2(-1, 180), ImGuiInputTextFlags_AllowTabInput);

                        if (ImGui::Button(xorstr("Execute (Simulate)"), ImVec2(200, 30))) {
                            // Simulator for the strings found in Trigger Finder
                        }
                        ImGui::SameLine();
                        if (ImGui::Button(xorstr("Load Last Found Triggers"), ImVec2(200, 30))) {
                            std::lock_guard<std::mutex> lock(Core::Features::cTriggerFinder::ResultsMutex);
                            std::string allTriggers;
                            for (auto& r : Core::Features::cTriggerFinder::Results) {
                                allTriggers += r.Text + "\n";
                            }
                            strncpy_s(buf, allTriggers.c_str(), sizeof(buf) - 1);
                        }

                        ImGui::TextColored(ImColor(150, 150, 150), xorstr("Note: External execution works by identifying the strings in memory."));
                        ImGui::TextColored(ImColor(150, 150, 150), xorstr("Native Lua execution requires an internal DLL."));
                    }
                    ImGui::EndCustomChild();
                }
                ImGui::EndGroup();
            ...
            case MISC_TROLLING:
                ...
                ImGui::BeginGroup();
                {
                    ...
                    ImGui::BeginGroup();
                    {
                        ...
                        {
                            if (SeletedIndex == -1) {
                                ImGui::TextColored(ImColor(150, 150, 150), xorstr("Select a player to troll"));
                            } else {
                                auto p = Core::SDK::Game::EntityList[SeletedIndex];
                                ImGui::TextColored(ImColor(255, 100, 100), p.NetworkInfo.UserName.c_str());
                                ImGui::Separator();
                                ImGui::Spacing();

                                if (ImGui::Button(xorstr("Force Ragdoll"), ImVec2(-1, 30))) {
                                    if (p.Ped) Mem.Write<int>((uintptr_t)p.Ped + g_Offsets.m_NoRagDoll, 0);
                                }
                                if (ImGui::Button(xorstr("Freeze Ped"), ImVec2(-1, 30))) {
                                    if (p.Ped) p.Ped->FreezePed(true);
                                }
                                if (ImGui::Button(xorstr("Unfreeze Ped"), ImVec2(-1, 30))) {
                                    if (p.Ped) p.Ped->FreezePed(false);
                                }
                                
                                ImGui::Spacing();
                                ImGui::TextColored(ImColor(150, 150, 150), xorstr("More options coming in v1.7.0"));
                            }
                        }
                        ImGui::EndCustomChild();
                    }
