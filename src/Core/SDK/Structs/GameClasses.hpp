...
public:
	...
	void SetRecoil( float Recoil ) {
		if ( !this ) { return; }
		CWeaponInfo * WeaponInfo = ( CWeaponInfo * ) GetWeaponInfo( );
		Mem.Write<float>( ( uintptr_t ) WeaponInfo + g_Offsets.m_Recoil, Recoil );
	}
	...
	void SetSpread( float Spread ) {
		if ( !this ) { return; }
		CWeaponInfo * WeaponInfo = ( CWeaponInfo * ) GetWeaponInfo( );
		Mem.Write<float>( ( uintptr_t ) WeaponInfo + g_Offsets.m_Spread, Spread );

	}

	void BulletDamage(float Boost) {
		if (!this) { return; }
		CWeaponInfo* WeaponInfo = (CWeaponInfo*)GetWeaponInfo();
		Mem.Write<float>((uintptr_t)WeaponInfo + 0xB0, Boost);
	}
