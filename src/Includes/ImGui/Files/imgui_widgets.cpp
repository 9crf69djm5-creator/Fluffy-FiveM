...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    if (Enabled)
    {
        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(75, 70, 175, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex)
{
    ...
    ImColor BGColor = enabled ? ImColor(75, 70, 175, (int)(GetStyle().Alpha * 255)) : ImColor(16, 16, 16, (int)(GetStyle().Alpha * 255));
    ...
    if (enabled)
        window->DrawList->AddRectFilled(ImVec2(window->Pos.x + Value->second.posx + 1, bb.Min.y + 1), ImVec2(window->Pos.x + Value->second.posx + size.x - 1, bb.Max.y - 1), ImColor(75, 70, 175, (int)(GetStyle().Alpha * 255)), 5, Corner);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_Y)
        window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, ImColor(75, 70, 175, 0), ImColor(75, 70, 175, 0), ImColor(75, 70, 175, (int)(GetStyle().Alpha * 255)), ImColor(75, 70, 175, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    anim.a = ImLerp(anim.a, *v ? 0.0f : 1.0f, g.IO.DeltaTime * 8.0f);
    ...
    window->DrawList->AddRectFilled(
        ...
        ImColor(14, 14, 14, (int)(GetStyle().Alpha * 255)),
        5
    );

    float anim_scale = 1.0f - anim.a;
    ImVec2 center = (check_bb.Min + check_bb.Max) * 0.5f;
    ImVec2 size = (check_bb.Max - check_bb.Min) * 0.5f * anim_scale;

    if (anim_scale > 0.01f)
    {
        window->DrawList->AddRectFilled(
            center - size + ImVec2(1, 1),
            center + size - ImVec2(1, 1),
            ImColor(184, 51, 142, (int)(GetStyle().Alpha * 255)),
            5
        );
    }
    ...
    if (anim_scale > 0.10f)
    {
        float checkmark_size = (square_sz - 12) * anim_scale;
        RenderCheckMark(
            window->DrawList,
...
struct KeyBindAnim
{
    float a;
    ImVec4 b;
    float radiusAnim = -1.f; // ← novo campo para animar o raio do círculo
};
...
bool ImGui::KeyBind(const char* label, int* Key, int* KeyState)
{
    ...
    float radius = 2.0f;
    ImVec2 circleCenter = ImVec2(frame_bb.Max.x - 10, frame_bb.Min.y + frame_bb.GetHeight() / 2);
    window->DrawList->AddCircleFilled(circleCenter, radius, IM_COL32(184, 51, 142, (int)(GetStyle().Alpha * 255)), 360);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), 5);
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2), Value->second.a, ImColor(184, 51, 142, (int)(GetStyle().Alpha * 255)), 360);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 0), ImVec2(frame_bb.Min.x + Value->second.anim + 1, frame_bb.Max.y), ImColor(184, 51, 142, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec4 col_rgb_no_alpha(col_rgb.x, col_rgb.y, col_rgb.z, 1.0f);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(
            ...
            GetColorU32(col_rgb_no_alpha), rounding, ImDrawFlags_RoundCornersAll);
    }
    else
    {
        ImVec4 col_src = (flags & ImGuiColorEditFlags_AlphaPreview) ? col_rgb : col_rgb_no_alpha;
        if (col_src.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(
                window->DrawList, bb_inner.Min, bb_inner.Max,
                GetColorU32(col_src), grid_step, ImVec2(off, off),
                rounding, ImDrawFlags_RoundCornersAll);
        else
            window->DrawList->AddRectFilled(
                bb_inner.Min, bb_inner.Max,
                GetColorU32(col_src), rounding, ImDrawFlags_RoundCornersAll);
    }
    ...
    {
        ColorTooltip(desc_id, &col.x, flags & (
